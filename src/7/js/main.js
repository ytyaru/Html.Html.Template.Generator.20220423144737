...
function generate() {
    const NOW = new Date(Date.now() + 9 * 3600000).toISOString().replace(/\..*/, '+09:00'); // https://qiita.com/mozukuzuku/items/68296fd9e764d95bb371
    // meta
    ...
    og.Options.article.published_time = NOW 
    og.Options.article.modified_time = NOW 
    delete og.Options.article.expiration_time
    // Google schema.org
    const schema = Htpl.SchemaOrg.Article;
    schema.Options.author.name = author
    schema.Options.dateModified = NOW
    schema.Options.datePublished = NOW
    ...
    html.push(`<script type="${schema.Mime}">`)
    html.push(schema.generate())
    html.push('</script>')
    html.push('</head>')
    html.push('<body>')
    html.push('</body>')
